WITH RECURSIVE segments AS (
    ...
    SELECT MAX(extract(epoch from tracks.time))/20 AS interval, MAX(extract(epoch from tracks.time)) AS max
    FROM tracks
    ...
    SELECT extract(epoch from tracks.time) AS duration, interval
...
SELECT COUNT(filtered_tracks.*), numbers.n
    ...
    RIGHT OUTER JOIN numbers ON (filtered_tracks.duration >= numbers.n AND filtered_tracks.duration < numbers.n+filtered_tracks.interval)
    GROUP BY numbers.n
    ORDER BY numbers.n