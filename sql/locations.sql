...
SELECT tracks.start_name, 0 AS cnt, locations.the_geom
FROM tracks JOIN locations ON locations.cartodb_id=start_id
WHERE
    tracks.id IN (
        SELECT id FROM tracks
        WHERE tracks.user_id = 1 
            AND (tracks.end_name='Pq Quinta das Conchas') 
    )
GROUP BY locations.cartodb_id
ORDER BY cnt DESC
UNION
SELECT tracks.end_name, COUNT(*) AS cnt, locations.the_geom
FROM tracks JOIN locations ON locations.cartodb_id=end_id
WHERE
    tracks.id IN (
        SELECT id FROM tracks
        WHERE tracks.user_id = 1 
