...
SELECT ST_x(geom) AS lng, ST_y(geom) AS lat, count(geom) AS count
FROM (
	SELECT ST_SnapToGrid(points.the_geom,0.0001) AS geom FROM points 
	WHERE points.track_id IN (
		SELECT tracks.id
		FROM tracks
		WHERE tracks.user_id = 1
			AND tracks.carbon >= 6000
	        AND tracks.id IN 
	        	(SELECT segments.track_id FROM segments WHERE segments.transport_mode='Walk')
	)
) AS foo GROUP BY lng, lat